...
import MatchGameItem from './component/MatchGameItem'
import './App.css'
...
class App extends Component {
  ...
  onImages = id => {
    ...
    const filter = imagesList.filter(eachItem => eachItem.imageUrl === imgUrl)
    if (filter[0].id === id) {
  ...
  render() {
    const {category, imgUrl, score, sec, isActive} = this.state
        ...
        {isActive ? (
          <div>
            <div className="random-img-container">
              <img src={imgUrl} alt="random-img" className="random-img" />
            </div>
